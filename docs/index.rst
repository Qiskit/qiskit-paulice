...
A check is comprised of a number of controlled Pauli rotations, :math:`P`, placed on some wires in
the circuit, :math:`w`: :math:`\{(P_1,w_1), ..., (P_k,w_k)\}`. For a given check, the controls lie
on a single ancilla qubit, and the rotations occur on wires of a single target qubit. For Clifford
circuits, a check is valid if its backpropagated product is a stabilizer of the state prepared by
the ideal circuit: :math:`\prod_{i}B(P_i,w_i) \in S`, where :math:`B(P,w)` is the backpropagator
...
- While many stochastic steps in the algorithm are controllable with a random seed, some features
  ...
  ``add_pauli_checks`` will cause undeterministic check picking: ``cost="LER"``,
...
The source code is available `on GitHub <https://github.com/Qiskit/qiskit-paulice>`_.

The developer guide is located at `CONTRIBUTING.md <https://github.com/Qiskit/qiskit-paulice/blob/main/CONTRIBUTING.md>`_
...
improvement, breaking change, or deprecation will be documented in the `release notes <https://quantum.cloud.ibm.com/docs/en/api/qiskit-addon-paulice/release-notes>`_.
